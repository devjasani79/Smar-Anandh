  ...
  return (
    ...
    >
        ...
        <div className="flex-1 min-w-0">
          ...
          {member.is_emergency_contact && (
            <span className="inline-flex items-center gap-1 text-sm bg-destructive/10 text-destructive px-2 py-0.5 rounded-full mt-1">
              <Heart className="w-3 h-3" /> Emergency
            </span>
          )}
        </div>
      </div>
      ...
      <div className="flex gap-2">
        <ActionBtn icon={<Phone className="w-6 h-6" />} label="Call" onClick={onCall} className="bg-primary text-primary-foreground" />
        <ActionBtn icon={<Video className="w-6 h-6" />} label="Video" onClick={onVideoCall} className="bg-success text-success-foreground" />
        <ActionBtn icon={<MessageCircle className="w-6 h-6" />} label="Message" onClick={onMessage} className="bg-muted text-muted-foreground" />
      </div>
    </motion.div>
  );
}
