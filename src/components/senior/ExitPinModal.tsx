...
export function ExitPinModal({ isOpen, onClose, onSuccess, seniorId }: ExitPinModalProps) {
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-foreground" style={{ fontFamily: 'Nunito, sans-serif' }}>
                    PIN Daaalein
                  </h2>
                  <p className="text-sm text-muted-foreground" style={{ fontFamily: 'Nunito, sans-serif' }}>
                    Baahar jaane ke liye PIN daaalein
                  </p>
                </div>
              </div>
            ...
            {error && (
              <p className="text-destructive text-center text-sm mb-4" style={{ fontFamily: 'Nunito, sans-serif' }}>
                {error}
              </p>
            )}
            ...
            <div className="grid grid-cols-3 gap-2">
              {digits.map((digit, index) => {
                if (digit === '') return <div key={index} className="w-16 h-14" />;
                if (digit === 'del') {
                  return (
                    <motion.button key={index} whileTap={{ scale: 0.95 }} onClick={handleDelete}
                      disabled={isLoading || pin.length === 0}
                      className="w-16 h-14 mx-auto rounded-xl bg-muted text-muted-foreground flex items-center justify-center disabled:opacity-50">
                      ←
                    </motion.button>
                  );
                }
                return (
                  <motion.button key={index} whileTap={{ scale: 0.95 }} onClick={() => handleDigit(digit)}
                    disabled={isLoading}
                    className="w-16 h-14 mx-auto rounded-xl bg-card text-foreground text-xl font-semibold shadow-sm active:shadow-none disabled:opacity-50 transition-all"
